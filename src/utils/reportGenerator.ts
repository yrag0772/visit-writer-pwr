...
export const generateVisitReport = (record: VisitRecord): string => {
  ...
  record.childStatuses.forEach((c) => {
    ...
    if (c.sleepStatus?.includes('趴睡（應宣導趴睡風險及五招安心睡）')) followUpItems.push({ target: name, label: '睡眠情形', status: '趴睡（應宣導趴睡風險及五招安心睡）' });
    if (c.sleepPosture === '一歲以下幼兒非仰睡（應輔導）') followUpItems.push({ target: name, label: '睡姿', status: '一歲以下幼兒非仰睡（應輔導）' });
  ...
  const visitTypeInfo = [
    ...
    record.visitCountDesc ? `► 說明：${record.visitCountDesc}` : ''
  ].filter(Boolean).join('<br/>');
